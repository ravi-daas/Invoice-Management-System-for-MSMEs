...
export default function QuotationFormModify() {
  const { selectedQuotation, updateQuotation } = useQuotations();
  const [quotationDate, setQuotationDate] = useState(
    selectedQuotation?.quotationDate
  );
  const [quatationNo, setQuatationNo] = useState("A000001");
  const [dueDate, setDueDate] = useState(selectedQuotation?.dueDate);
  const [logo, setLogo] = useState<string | null>(null);
  const { quotations } = useQuotations();
  const [selectedClient, __] = useState<ClientData | null>(
    selectedQuotation?.billedTo || null
  );
  const [showSignatureUpload, setSignatureShowUpload] = useState(false);
  const [openAddGST, setOpenAddGST] = useState(false);
  const [gstType, setGstType] = useState(selectedQuotation?.gstType || "");
  ...
  const [businesses, _] = useState<ClientData | null>(
    selectedQuotation?.billedTo || null
  );

  console.log("Selected Quotation: ", selectedQuotation?.billedTo);
      ...
      <div className="mx-auto max-w-5xl space-y-8  border border-gray-200 rounded-lg p-4 bg-white dark:bg-black">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex items-baseline justify-between">
                <h2 className="text-lg font-semibold">Quotation For</h2>
                <span className="text-sm text-muted-foreground">
                  (Client's Details)
                </span>
              </div>
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="new" className="text-blue-500 font-medium">
                    <Plus className="h-4 w-4 mr-2 inline" /> Add New Client
                  </SelectItem>
                </SelectContent>
              </Select> */}
