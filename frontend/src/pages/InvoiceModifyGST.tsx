...
import { Calendar, ImageIcon, Plus, Upload, X, XIcon } from "lucide-react";
...
export default function InvoiceFormModifyRetail() {
  ...
  const [modeOfPayment, setModeOfPayment] = useState(
    selectedInvoice?.termOfPayment
  );
  const [otherRef, setOtherRef] = useState(selectedInvoice?.OtherRef);
  const [dispatchDocNo, setDispatchDocNo] = useState(
    selectedInvoice?.DispatchDocNo
  );
  const [deliveryNoteDate, setDeliveryNoteDate] = useState(
    selectedInvoice?.deliveryNoteDate
  );
  const [dispatchedThrough, setDispatchedThrough] = useState(
    selectedInvoice?.DispatchedThrough
  );
  const [refNo, setRefNo] = useState(selectedInvoice?.RefNo);
  const [destination, setDestination] = useState(selectedInvoice?.Destination);
  const [invoiceDate, setInvoiceDate] = useState(selectedInvoice?.invoiceDate);
  const [dueDate, setDueDate] = useState(selectedInvoice?.dueDate);
  const [logo, setLogo] = useState<string | null>(null);
  const [selectedClient, _] = useState<ClientData | null>(
    selectedInvoice?.billedTo || null
  );
  const [showSignatureUpload, setSignatureShowUpload] = useState(false);
  const [signature, setSignature] = useState<string | null>(null);
  const [currency, setCurrency] = useState(selectedInvoice?.currency || "inr");
  const [businesses, __] = useState<ClientData | null>(
    selectedInvoice?.billedBy || null
  );
  const [openAddGST, setOpenAddGST] = useState(false);
  const [gstType, setGstType] = useState(selectedInvoice?.gstType || "");
  ...
  const handleSaveAndContinue = () => {
    ...
    navigate("/InvoiceDashboard/AllInvoices");
  };
  ...
  return (
    <div className="p-6 bg-white dark:bg-black " id="invoice">
      <div className="mx-auto max-w-5xl space-y-8  border border-gray-200 rounded-lg p-4 bg-white dark:bg-black">
        ...
        <div className="space-y-2 flex flex-col items-center justify-center align-middle">
          <h1 className="text-3xl font-semibold text-[#67338a]">Invoice</h1>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-6">
            ...
            {/* Billed By Section */}
            <div className="space-y-4" style={{ marginTop: "35px" }}>
              <div className="flex items-baseline justify-between">
                <h2 className="text-lg font-semibold">Billed By</h2>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="new">Select a Client</SelectItem>
                  {clients.length > 0 &&
                  ...
                  <SelectItem
                    value="new"
                    className="text-[#67338a] font-medium"
                  >
        ...
        <div className="flex items-end gap-4 pt-4 justify-start">
          ...
          {/* <Input className="w-48" placeholder="Number/Currency Format" /> */}
        ...
        {items.map((item) => (
          <div className=" flex flex-col bg-purple-50 dark:bg-black dark:border dark:border-white dark:rounded p-2">
            ...
            <div className="flex gap-4">
              <Button variant="ghost" className="text-[#67338a]">
                <Plus className="h-4 w-4 mr-2" />
                Add Description
              </Button>
            </div>
            {/* <div className="flex gap-4 w-96 mt-4">
              <Button variant="ghost" className="text-[#67338a] w-32">
